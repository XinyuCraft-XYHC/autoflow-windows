...
#   【全面修复与优化】
#   修复：
#     - 检查更新弹窗逻辑：设置页「前往下载」按钮、状态栏「点此下载」均改为打开
#       更新对话框（UpdateDialog），而非直接跳外链；已忽略版本在状态栏仍显示点击按钮；
#     - 桌面快捷方式现弹出「另存为」对话框让用户选择保存位置（默认桌面），
#       不再强制保存到桌面；
#     - 快捷方式/命令行运行不了：exe 名改为固定 AutoFlow.exe，彻底解决路径不稳定问题；
#   优化：
#     - 任务编辑页顶部展示任务 ID（灰色小字）+「复制」按钮+「⌨ 命令行」弹窗按钮，
#       弹窗展示 ID 和完整命令行命令，均带一键复制；
#     - 软件图标优化：16×32px 小图标不渲染文字（只显示无限符号），
#       48px 以上才渲染 autoflow 文字，解决小尺寸图标文字巨大无比的问题；
#       16/32px 时无限符号居中，不再偏上；
#     - 主程序 exe 名固定为 AutoFlow.exe（不带版本号）：
#       * 安装包升级后快捷方式/开始菜单/PATH 命令无需重建
#       * 安装包新增「将 AutoFlow 添加到系统 PATH」可选任务（默认勾选），
#         安装后可在任意命令行直接执行 AutoFlow --run-task <id>
...
#   - 修复文字识别（选控件 OCR）始终提示"未识别到文字"（两处根本原因）：
#     1. 原子进程脚本使用 winsdk API（未安装），实际应使用 winrt-Windows.Media.Ocr 包（已安装）；
#        重写 _ocr_pixmap 子进程脚本，直接使用 winrt.windows.media.ocr 新版 API；
#     2. 多显示器 DPR 不一致（主屏 DPR=1.0 + 副屏 DPR=1.5）导致截图坐标计算错误：
#        原逻辑用主屏 DPR 创建 canvas，副屏物理截图(1080x1920)绘制到只容纳逻辑尺寸的 canvas 外；
#        修复：统一以逻辑像素为基准拼合全屏图（每屏截图先缩放到逻辑尺寸），
