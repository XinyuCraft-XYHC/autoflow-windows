...
AutoFlow 提供了灵活的插件系统，允许开发者编写自定义功能块和触发器来扩展应用的能力。本指南将帮助你快速上手插件开发。
...
- `min_app_version`: 插件依赖的最低 AutoFlow 版本
- `permissions`: 权限列表（支持 `"block"`、`"trigger"`、`"setting"`）
- `tags`: 插件标签（用于分类和搜索）
...
- **Linux**: `~/.config/xinyucraft/autoflow/plugins/`
- **macOS**: `~/Library/Application Support/XinyuCraft/AutoFlow/plugins/`
...
2. **内置插件目录**（随源码或 exe 发布）：`plugins/`（相对于应用目录）
...
1. **手动安装**: 下载插件文件夹，放入 `plugins/` 目录
2. **自动安装**（未来支持）: AutoFlow 内置插件市场
...
### Q: 版本 2.6.0 和 2.7.0 有什么区别？

A: 
- **v2.6.0**: 基础插件系统（功能块和触发器）
- **v2.7.0** (当前): 完整插件生态，支持插件启用/禁用、持久化状态、可视化管理
